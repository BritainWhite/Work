...
function switchSubTab(tab) {
  ...
  document.querySelectorAll('.init-sub-tab').forEach(el => el.style.display = 'none');
...
function loadIframe(type) {
  const linkMap = {
    today: "customDate",
    yesterday: "customDateYesterday",
    tomorrow: "customDateTomorrow"
  };
  const iframeMap = {
    today: "previewIframe",
    yesterday: "iframeYesterday",
    tomorrow: "iframeTomorrow"
  };

  const input = document.getElementById(linkMap[type]);
  ...
  const date = valid ? val : fallback;

  const url = `https://radapps3.wal-mart.com/Protected/CaseVisibility/ashx/Main.ashx?func=init&storeNbr=5307&businessDate=${date}`;
  document.getElementById(iframeMap[type]).src = url;
}

// Init input updates
["customDate", "customDateYesterday", "customDateTomorrow"].forEach(id => {
